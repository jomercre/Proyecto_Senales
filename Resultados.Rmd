...
# Funcion para asegurar 3 canales
...
# Función para eliminar los coeficientes wavelet
...
plot_err <- function(img,img_noise,max_lev=8,wf='la8',mode = 'all') {
  ...
  plot(1:max_lev, err, type = 'b', pch = 19, col = 'red',
     xlab = 'Niveles de la DWT', ylab = 'RMSE', 
     main = paste0('Error en función del nivel para wf = ', wf))
}

# Funcion de graficado de la reconstruccion
...
Aplicamos `plot_err`/`plot_comp` (con `opt_lev` calculado) a todas las imágenes
...
Generamos las gráficas del RMSE y la reconstuccion para cada imagen del directorio fotos_ruido.

```{r}
dir_out <- "resultados"
if (!dir.exists(dir_out)) dir.create(dir_out)

# Iteramos sobre todas las bases
...
# Diferencia entre tipos de ruido

Utilizamos la imagen de la vaca para ver como afectan los diferentes ruidos a una misma imagen.

```{r}
target_base <- "objeto_vaca"  
wf <- "la8"
max_lev <- 6
...
archivo_salida <- file.path("resultados", "resultados_vaca.csv")
    write.csv(results_vaca, archivo_salida, row.names = FALSE)

knitr::kable(results_vaca, caption = paste("Resultados para", target_base, "con estrategia 'ALL'"))
```

# Grafico de barras del RMSE promedio

Comparamos para la imagen de la vaca el valor del RMSE al reconstruir la imagen con el modo all y con el modo diag.
...
ggplot(datos_grafico, aes(x = noise, y = RMSE_Promedio, fill = Estrategia)) +
  ...
  scale_fill_manual(values = c("All" = "#4E84C4", "Diag" = "#D16103")) + 
  theme_minimal() +
  labs(
