---
title: Eliminación de ruido en imágenes mediante la transformada wavelet discreta
author:
...
+ Datos de entrada: 8 fotos propias, preprocesadas y normalizadas en rango [0,1].
...
En esta sección se detalla la procedencia del conjunto de datos y el tratamiento digital que se ha aplicado antes de la introducción del ruido sintético. La preparación de las imágenes ha sido un paso fundamental para asegurar la validez de los resultados posteriores.

## 3.1. Descripción del conjunto de datos

El conjunto de datos ha constado de 8 imágenes originales capturadas por el equipo. Se han seleccionado dos categorías de imágenes para evaluar el comportamiento del filtrado ante diferentes tipos de detalle:

* **Escenarios naturales (4 imágenes):** Incluyen capturas de cielos diurnos y nocturnos (`cielo_dia_nubes`, `cielo_noche`, etc.). Estas imágenes se caracterizan por presentar cambios de luz suaves y texturas naturales, lo que permite evaluar cómo se comporta el filtrado en zonas uniformes.
* **Objetos (4 imágenes):** Incluyen elementos con bordes muy definidos y un contraste alto (`objeto_gafas`, `objeto_vaca`, `objetos_juguetes`). Estas imágenes son esenciales para comprobar si la técnica DWT es capaz de mantener los bordes de los objetos sin difuminarlos demasiado.
...
```
## 3.2. Algoritmo de preprocesamiento y normalización

Las imágenes originales presentaban resoluciones y niveles de luz muy diferentes entre sí. Para unificar el experimento, se ha diseñado e implementado un algoritmo de preprocesado en R. Las transformaciones que se han aplicado son las siguientes:

1.  **Redimensionamiento:** Todas las imágenes se han reescalado a una resolución fija de **$1024 \times 1024$ píxeles**. Se ha elegido este tamaño por ser una potencia de 2 ($2^{10}$), lo cual resulta ideal para la Transformada Wavelet Discreta, ya que permite dividir la imagen sucesivamente hasta en 10 niveles sin problemas de bordes.

2.  **Normalización de intensidad:** Se ha observado que algunas imágenes originales tenían poco contraste. Para corregir esto, se ha aplicado una transformación matemática a cada píxel para ajustar sus valores al intervalo $[0,1]$ mediante la siguiente fórmula:

$$
I_{norm} = \frac{I - \min(I)}{\max(I) - \min(I)}
$$

Con este proceso, se ajustan las imágenes a un formato común y se normalizan las intensidades al rango $[0,1]$, para tener una misma estructura de canales y una escala común respectivamente. Al trabajar con imágenes en escala común, las medidas de RMSE pueden compararse entre fotos y tipos de ruido, y la normalización evita que valores de intensidad fuera del rango marcado condicionen la DWT o la reconstrucción de las fotos. Finalmente, se almacenan las imágenes preprocesadas en un directorio de salida (`fotos_clean/`) siguiendo una convención de nombres consistente (`clean_<nombre>.jpg`), que posteriormente facilita el emparejado con sus versiones ruidosas.
...
Para facilitar el procesamiento en batch y el emparejado automático clean/noisy, cada archivo generado sigue la convención: `<nombre>_gauss.jpg`, `<nombre>_speckle.jpg`, `<nombre>_poisson.jpg`, `<nombre>_sp.jpg` (sal y pimienta) donde `<nombre>` es el identificador de la imagen original (sin extensión). Esta convención es clave porque permite, a partir del nombre del archivo, recuperar qué imagen limpia corresponde (`clean_<nombre>.jpg`), y qué tipo de ruido se aplicó.
...
En cada nivel de descomposición se obtiene una componente de aproximación (baja frecuencia) LL y tres de detalle (alta frecuencia) HL, LH y HH, a diferencia de en una dimensión donde solo se obtenía un coeficiente de aproximación y uno de detalle. Esta diferencia reside en que en dos dimensiones podemos aplicar un filtro distinto en cada dirección del espacio, lo que nos da lugar a las siguientes combinaciones:

+ Filtro paso bajo en la dirección de las columnas y paso bajo en la de las filas $\Longrightarrow$ LL

+ Filtro paso bajo en la dirección de las filas y paso alto en la de las columnas $\Longrightarrow$ LH

+ Filtro paso alto en la dirección de las filas y paso bajo en la de las columnas $\Longrightarrow$ HL

+ Filtro paso alto en la dirección de las filas y paso alto en la de las columnas $\Longrightarrow$ HH

Por ello, una forma directa de reducir el ruido es anular o atenuar los coeficientes de detalle y reconstruir la imagen de nuevo. El único inconveniente que se tiene al anular dichas componentes de detalle la calidad de la imagen (bordes y texturas) se puede ver afectada considerablemente.
...
knitr::include_graphics(c(
  "resultados jose/cielo_dia_nubes_gauss_rmse_all.jpg"
))
```
...
knitr::include_graphics(c(
  "resultados jose/cielo_dia_nubes_gauss_rmse_all.jpg",
  "resultados jose/cielo_dia_nubes_gauss_rmse_diag.jpg"
))
```
...
knitr::include_graphics(c(
  "resultados jose/cielo_dia_nubes_speckle_rmse_all.jpg",
  "resultados jose/cielo_dia_nubes_speckle_rmse_diag.jpg"
))
```
...
knitr::include_graphics(c(
  "resultados jose/cielo_dia_nubes_poisson_rmse_all.jpg",
  "resultados jose/cielo_dia_nubes_poisson_rmse_diag.jpg"
))
```
...
knitr::include_graphics(c(
  "resultados jose/cielo_dia_nubes_sp_rmse_all.jpg",
  "resultados jose/cielo_dia_nubes_sp_rmse_diag.jpg"
))
```
...
## 7.1. Diferencia entre tipos de ruido
...
Para poder hacer una comparación relevante, hemos calculado el RMSE promedio que genera cada estrategia para cada tipo de ruido _figura 15_ y podemos apreciar que la estrategia "all" produce un error promedio para todos los tipos de ruido menor que la estrategia "diag", por lo que es preferible utilizar la primera a la segunda en la mayoría de situaciones. Aun así, analizar los casos específicos nos dará información muy valiosa, como veremos en la sección siguiente.

\vspace{0.5cm}

## 7.3. Comparación de estrategias de eliminación de coeficientes en casos extremos de Sal y Pimienta 
...
## 7.4. Comparación entre tipos de imagen
...
Lo primero que observamos es como las imágenes alcanzan su reconstrucción óptima para niveles de la DWT distintos, mientras que las imágenes con objetos lo hacen en el nivel 2, las imágenes del cielo se quedan en los niveles 4 para el día y 3 para la noche. Esto se debe a que las imágenes del cielo suelen ser más monocromáticas por lo que aumentar el nivel de profundidad de la wavelet utilizada, es decir, perder calidad y confundir los contornos es favorable desde el punto de vista del error. Mientras que las imágenes con objetos suelen tener una gama de colores más varidada lo que lleva a que pequeñas variaciones en los contornos aumenten significativamente el RMSE.
...
```
\vspace{0.5cm}
```{r, echo=FALSE, out.width="49%", fig.cap="Comparación de imágenes con composiciones distintas - Tipo de ruido Gaussiano - Modo 'diag'" ,fig.align='center', fig.show='hold'}
knitr::include_graphics(c(
...
En resumen, la elección del nivel óptimo de DWT depende fundamentalmente del contenido de la imagen: mientras las escenas homogéneas (cielo) se benefician de descomposiciones más profundas para maximizar la eliminación de ruido, las escenas con objetos definidos requieren niveles más superficiales para preservar la información estructural crítica.
