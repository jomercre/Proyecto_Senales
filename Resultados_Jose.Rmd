...
archivo_salida <- file.path("resultados jose", "resultados_vaca.csv")
...
for (base_name in mis_imagenes) {
  ...
  if (length(f_ruido) == 0) {
    message(paste("No encuentro archivo con ruido", mi_ruido, "para", base_name))
    next
  }
  f_ruido <- f_ruido[1]
  ...
  if (!file.exists(f_clean)) {
    message(paste("No existe la imagen limpia:", f_clean))
    next
  }
  ...
  #  Calcular errores para AMBOS métodos
  err_all <- get_err(img, img_noise, max_lev=max_lev, wf=wf, mode="all")
